import ZdogScene from './ZdogScene';

const illoSize = 128;
const zoom = 1;

function renderScene(Zdog) {
  ...
  (function () {
    ...
    const rightHandPosition = new Zdog.Vector(rightWrist).add({
      x: -0.5,
      z: -1,
    });
...
function animate(Zdog, illo, ticker) {
  const progress = ticker / 240;
  const theta = Zdog.easeInOut( progress % 1 ) * Zdog.TAU;
  illo.rotate.y = -theta + -Zdog.TAU / 8;
  illo.updateGraph();
  return [ticker+1, illo];
}

const Demo = (props) => {
  return <ZdogScene scene={renderScene} animate={animate} width={illoSize} height={illoSize} zoom={zoom}/>
};
